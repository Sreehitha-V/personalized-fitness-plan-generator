...
FIXES APPLIED:
  1. Wrong dashboard URL fixed (was 'LakshmiNandaS/FitPlanAI_PLAN_dulpicate' typo)
  2. Python 3.10 f-string ternary-with-quotes bug fixed (pre-assigned bar_color)
  3. Email lookup now uses get_user_profile() not get_user_setting()
  4. Missed count logic fixed — skipped days no longer counted as missed
  5. get_water() None guard added — prevents TypeError crash
  6. td margin:4px replaced with padding (ignored by Gmail/Outlook)
"""
import os, requests
...
def _build_email_html(uname, stats, plan_info, week_str):
    ...
    # FIX 2: pre-assign bar_color so no ternary inside f-string (Python 3.10 safe)
    bar_color = "#22c55e" if pct >= 80 else "#fbbf24"
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%">
  ...
  <!-- HEADER -->
  <tr><td style="background:#1a0608;border:1px solid rgba(229,9,20,0.30);border-radius:16px 16px 0 0;padding:32px 36px;text-align:center">
  ...
  <!-- STATS -->
  <!-- FIX 6: removed margin:4px from <td> (ignored by Gmail/Outlook). Use padding instead. -->
  <tr><td style="background:#0f0608;border-left:1px solid rgba(229,9,20,0.20);border-right:1px solid rgba(229,9,20,0.20);padding:0 36px 24px">
    ...
    <tr>
      <td width="22%" style="text-align:center;padding:12px 8px;background:rgba(229,9,20,0.08);border:1px solid rgba(229,9,20,0.18);border-radius:12px">
        <div style="font-size:2rem;font-weight:900;color:#22c55e">{done}</div>
        <div style="font-size:0.65rem;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,0.45);margin-top:3px">Done</div>
      </td>
      <td width="4%"></td>
      <td width="22%" style="text-align:center;padding:12px 8px;background:rgba(229,9,20,0.08);border:1px solid rgba(229,9,20,0.18);border-radius:12px">
      ...
      <td width="22%" style="text-align:center;padding:12px 8px;background:rgba(229,9,20,0.08);border:1px solid rgba(229,9,20,0.18);border-radius:12px">
        ...
        <div style="font-size:0.65rem;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,0.45);margin-top:3px">Streak</div>
      </td>
    </tr>
    </table>
  </td></tr>
  ...
  <!-- PROGRESS BAR -->
  <!-- FIX 2: bar_color pre-assigned above — no ternary inside f-string -->
  <tr><td style="background:#0f0608;border-left:1px solid rgba(229,9,20,0.20);border-right:1px solid rgba(229,9,20,0.20);padding:0 36px 28px">
    ...
    <div style="height:10px;background:rgba(255,255,255,0.08);border-radius:5px;overflow:hidden">
      <div style="height:100%;width:{pct}%;background:linear-gradient(90deg,#E50914,{bar_color});border-radius:5px"></div>
    </div>
  ...
  <!-- MOTIVATION -->
  <tr><td style="background:#0f0608;border-left:1px solid rgba(229,9,20,0.20);border-right:1px solid rgba(229,9,20,0.20);padding:20px 36px;text-align:center">
  ...
  <!-- CTA -->
  <!-- FIX 1: URL corrected to actual HuggingFace space -->
...
def send_weekly_summary(username, user_data, session_state, to_email=None):
    ...
    # FIX 4: missed only counts pending days (not skipped, not done)
    done = missed = total = 0
    for i, sd in enumerate(sdays):
        ...
        if week_start <= work_date <= week_end and not sd.get("is_rest_day"):
            ...
            if status == "done":
                ...
                pass  # skipped is a choice, not a miss
            elif work_date < today:
                missed += 1  # past + no status = genuinely missed
    ...
    # FIX 5: guard against get_water returning None
    avg_water = 0
    try:
        from utils.db import get_water
        waters = []
        for d in range(7):
            _d = (week_start + timedelta(days=d)).isoformat()
    ...
    # FIX 3: email from user_profiles table, not user_settings
    if not to_email:
        to_email = user_data.get("email", "")
    if not to_email:
        try:
            from utils.db import get_user_profile
            _profile = get_user_profile(username)
            if _profile:
                to_email = _profile.get("email", "")
        except Exception:
            pass
    if not to_email:
    ...
    payload = {
        ...
        "subject":     f"FitPlan Pro - Weekly Summary ({week_str})",
